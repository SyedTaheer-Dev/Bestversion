          ...
          <div className="max-w-xl space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ...
                <motion.div key={item.title} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + index * 0.08 }} className="rounded-3xl border border-border/60 bg-card/75 p-4 shadow-[0_12px_38px_rgba(0,0,0,0.16)] backdrop-blur-xl">
              ...
              <div className="mb-5 rounded-2xl border border-primary/20 bg-primary/8 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-6 text-muted-foreground">Use the default admin account to open the admin panel directly.</p>
            ...
            <div className="mb-6 grid grid-cols-4 gap-2 rounded-2xl border border-border/60 bg-background/60 p-1">
              {[
                ...
                <button
                  ...
                  className={`rounded-xl px-3 py-2 text-xs font-medium transition ${mode === item.id ? "bg-gradient-to-r from-primary to-secondary text-primary-foreground shadow-lg" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {item.label}
                </button>
              ))}
              ...
              >
                ...
                {mode === "login" && (
                  <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <div className="relative">
                      ...
                      <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required className={inputClass} />
                      <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3.5 top-3.5 text-muted-foreground hover:text-foreground">
                        {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                    <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-primary to-secondary py-3.5 text-sm font-semibold text-primary-foreground shadow-[0_12px_35px_hsl(var(--primary)/0.24)] transition hover:opacity-95 disabled:opacity-60">
                ...
                {mode === "signup" && (
                  <form onSubmit={handleSignup} className="space-y-4">
                    ...
                    <div className="relative">
                      <Lock size={18} className="absolute left-3.5 top-3.5 text-muted-foreground" />
                      <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className={inputClass} />
                      <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3.5 top-3.5 text-muted-foreground hover:text-foreground">
                        {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                ...
                {mode === "forgot" && (
                  <>
                    <div className="mb-5 space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">We’ll send a reset link by email, or provide a preview link locally if SMTP is not configured.</p>
                    </div>
                ...
                {mode === "phone" && (
                  <>
                    <div className="mb-5 space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">Request a 6-digit OTP and verify it to start a secure cookie session.</p>
                    </div>
                    ...
                    {!otpSent ? (
                      <form onSubmit={handleSendOtp} className="space-y-4">
                        <div className="relative">
                          <User size={18} className="absolute left-3.5 top-3.5 text-muted-foreground" />
                          <input type="text" placeholder="Full name (optional)" value={fullName} onChange={(e) => setFullName(e.target.value)} className={inputClass} />
                        </div>
                        <div className="relative">
                          <Smartphone size={18} className="absolute left-3.5 top-3.5 text-muted-foreground" />
                          <input type="tel" placeholder="Phone number" value={phone} onChange={(e) => setPhone(e.target.value)} required className={inputClass} />
                        </div>
                        <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-primary to-secondary py-3.5 text-sm font-semibold text-primary-foreground shadow-[0_12px_35px_hsl(var(--primary)/0.24)] transition hover:opacity-95 disabled:opacity-60">
                          {loading ? <Loader2 className="animate-spin" size={18} /> : null}
                      ...
                      <form onSubmit={handleVerifyOtp} className="space-y-5">
                        ...
                        <div>
                          <label className="mb-3 block text-xs font-medium uppercase tracking-[0.22em] text-muted-foreground">Enter OTP</label>
                          <InputOTP maxLength={6} value={otp} onChange={setOtp} containerClassName="justify-center">
                            <InputOTPGroup>
                              {Array.from({ length: 6 }).map((_, index) => (
                                <InputOTPSlot key={index} index={index} className="h-12 w-11 rounded-xl border border-border bg-muted/50 first:rounded-xl first:border last:rounded-xl" />
                              ))}
                            </InputOTPGroup>
                          </InputOTP>
                        </div>
                        <div className="flex gap-3">
                          <button type="button" onClick={resetPhoneFlow} className="flex-1 rounded-2xl border border-border/60 bg-background/70 py-3 text-sm font-medium text-foreground transition hover:bg-muted/60">
                            Change number
                          </button>
                          <button type="submit" disabled={loading} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-primary to-secondary py-3 text-sm font-semibold text-primary-foreground shadow-[0_12px_35px_hsl(var(--primary)/0.24)] transition hover:opacity-95 disabled:opacity-60">
                            {loading ? <Loader2 className="animate-spin" size={18} /> : null}
                            Verify OTP
                          </button>
                        </div>
